...
impl InvocationArgs {
    ...
    pub fn validate_against_schema(&self, schema: &Value) -> Result<(), ToolError> {
        validate_value_against_schema("$", &self.0, schema)
    }
}
...
fn validate_value_against_schema(
    ...
    if let Some(required) = schema.get("required").and_then(Value::as_array) {
        let object = value.as_object().ok_or_else(|| {
            ToolError::Validation(format!(
                "Argument {} must be an object to satisfy required fields",
                path
            ))
        })?;
    ...
    if let Some(properties) = schema.get("properties").and_then(Value::as_object) {
        let object = value.as_object().ok_or_else(|| {
            ToolError::Validation(format!(
                "Argument {} must be an object to validate properties",
                path
            ))
        })?;
        for (property_name, property_schema) in properties {
            if let Some(property_value) = object.get(property_name) {
                ...
                validate_value_against_schema(&property_path, property_value, property_schema)?;
            }
        }
    }
